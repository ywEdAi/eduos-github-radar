...
export default function RadarClient({ payload, skillsPayload, supportUrl }) {
  ...
  return <main className="radar"><header className="masthead"><div className="masthead-top"><Link href="/" className="wordmark">EDUOS <span>Radar</span></Link><nav className="catalog-nav" aria-label={copy.catalogLabel}><button aria-pressed={catalog === "apps"} onClick={() => selectCatalog("apps")}>{copy.apps}</button><button aria-pressed={catalog === "skills"} onClick={() => selectCatalog("skills")}>{copy.skills}</button><button aria-pressed={catalog === "data"} onClick={() => selectCatalog("data")}>{copy.data}</button></nav><div className="masthead-tools"><span className="sync"><i />{copy.sync} {payload.generated_at?.slice(0, 10)}</span><div className="language-switch"><button className={locale === "en" ? "active" : ""} onClick={() => switchLocale("en")}>EN</button><button className={locale === "zh" ? "active" : ""} onClick={() => switchLocale("zh")}>中文</button></div>{supportUrl ? <a className="support-link" href={supportUrl} target="_blank" rel="noreferrer">{copy.support} ↗</a> : <a className="support-link" href="#add-signal">{copy.submitProject} ↓</a>}</div></div><div className="masthead-line"><p>{copy.thesis}</p><p>{copy.thesisMore}</p></div></header>
    ...
    <SuggestionForm copy={copy} /><footer className="footer-mast"><span>EduOS Radar — a project by edu-ai-builders</span><span>{copy.footerMetadata}</span><Link href="/support">{copy.support} →</Link></footer>
  </main>;
}
